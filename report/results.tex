...
\end{equation}
the $F_1$ measure is the harmonic average between the two (in the previous formula we have \textit{TP = true positive}, \textit{FP = false positive}, \textit{FN = false negative}). In particular, since we deal with a multi-class problem we need to add a measure of weights to the $F_1$ equation:
\begin{equation}
...
Finally, as we can see, for task A we can say that using the \textit{two-step} approach definitely helped performances.